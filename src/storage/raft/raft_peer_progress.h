...
public:
    ...
    void reset_for_leader(const RaftMembership& membership, LogIndex last_log_index);
    void update_snapshot_watermark(ReplicaID replica_id, LogIndex snapshot_watermark);
    ...
    void handle_append_ok(ReplicaID replica_id, LogIndex prev_log_index, size_t entries_size);
    void handle_append_failed(ReplicaID replica_id, LogIndex follower_last_log_index, LogIndex leader_last_log_index);
...
private:
    ...
    std::unordered_map<ReplicaID, LogIndex, ReplicaIDHash> inflight_snapshot_index_;
};
