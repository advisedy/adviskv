...
ReplicaReadIndexChecker::ReplicaReadIndexChecker(ReplicaContext& context,
...
Status ReplicaReadIndexChecker::check_self_leader_and_get_read_index(
    ...
    RETURN_IF_INVALID_STATUS(raft_loop_.sync_submit_task([&] {
        RETURN_IF_INVALID_STATUS(
            context_.raft_node.build_append_entries_for_read(
    ...
    for (const RaftMessage& msg : read_effects.messages) {
        ...
        if (msg.type == RaftMessageType::APPEND_ENTRIES) {
            ...
            Status status = raft_loop_.sync_send_append_entries(
            ...
            RETURN_IF_INVALID_STATUS(raft_loop_.sync_handle_append_response(
            ...
            Status status = raft_loop_.sync_send_install_snapshot(
                msg.target, msg.snapshot_param, res);
            if (status.fail()) {
                RETURN_IF_INVALID_STATUS(
                    raft_loop_.sync_handle_install_snapshot_send_failed(
                        msg.target.replica_id, msg.snapshot_param, status))
                continue;
            }
            RETURN_IF_INVALID_STATUS(
                raft_loop_.sync_handle_install_snapshot_response(
