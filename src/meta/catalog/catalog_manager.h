...
public:
    ...
    Status alter_table_replica_count(const AlterTableReplicaCountParam& param, TableMeta* table_meta);
    Status delete_db(DatabaseID db_id);
    Status delete_table(TableID table_id, TableMeta* table_meta = nullptr);
    Status update_table_state(TableID table_id, TableState state, const std::string& last_error_msg = "");
    ...
    Status list_tables_by_state(TableState state, std::vector<TableMeta>* table_meta_list);
