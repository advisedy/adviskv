...
Status CatalogManager::get_table_by_id(TableID table_id, TableMeta* table_meta){
    ...
    return Status{StatusCode::NOT_SUPPORTED, "get_table_by_id is not implemented yet" };
}

Status CatalogManager::get_table_by_name(const std::string& db_name, const std::string& table_name, TableMeta* table_meta){
    std::shared_lock lock(mutex_);
...
Status CatalogManager::lookup_table_by_name(const std::string& db_name, const std::string& table_name, TableMeta* table_meta){

    auto it = db_table_name2table_id_.find(db_name);
    if(it == db_table_name2table_id_.end()){
        return Status{StatusCode::TABLE_NOT_FOUND, fmt::format("table_name:{} does not exist in db:{}", table_name, db_name)};
    }
...
// bool CatalogManager::lookup_table_by_id(const std::string& db_name, const std::string& table_name, TableMeta* table_meta){
