...
public:
    ...
    static Status encode_to_fd(int fd, const ObjectType& obj) {
        ...
        if (payload.size() > static_cast<size_t>(codec.max_payload_len())) {
            return Status{StatusCode::ERROR,
                          fmt::format("payload too large: {}, max={}", payload.size(), codec.max_payload_len())};
        }
        ...
        RETURN_IF_INVALID_STATUS(
                func::write_full(fd, payload.data(), payload.size(), "framed_record.payload_write.after_partial_write"))
    ...
    static Status decode_from_fd(int fd, ObjectType& obj, size_t& consumed_bytes) {
        consumed_bytes = 0;
        Codec codec;
        LenType total_len = 0;
        ...
        if (total_len <= 0) {
            return Status{StatusCode::ERROR, fmt::format("invalid total_len: {}", total_len)};
        }
        if (total_len > codec.max_payload_len()) {
            return Status{StatusCode::ERROR,
                          fmt::format("payload too large: {}, max={}", total_len, codec.max_payload_len())};
        }
        ...
        consumed_bytes = sizeof(LenType) + sizeof(uint32_t) + static_cast<size_t>(total_len);
        return Status::OK();
    }
};
