...
Status RouteManager::update_table_meta(const TableMetaCache& meta){
...
Status RouteManager::get_table_meta(const std::string& db_name, const std::string& table_name, TableMetaCache* out) const{
    std::shared_lock lock{caches_mutex_};
    TableMetaCacheKey key{db_name, table_name};
    auto it = table_meta_caches_.find(key);
    if(it == table_meta_caches_.end()){
        return Status{StatusCode::TABLE_META_CACHE_NOT_FOUND, fmt::format("db_name: {}, table_name: {} not found in cache", db_name, table_name)};
    } else{
        *out = it->second;
    }
    return Status::OK();
...
Status RouteManager::get_route(TableID table_id, ShardID shard_id, ShardRoute* out) const{
    std::shared_lock lock{routes_mutex_};
    ShardRouteKey key{table_id, shard_id};
    auto it = routes_.find(key);
    if(it == routes_.end()){
        return Status{StatusCode::ROUTE_NOT_FOUND, fmt::format("table_id: {}, shard_id: {} not found route", table_id, shard_id)};
    } else{
        *out = it->second;
    }
    return Status::OK();
