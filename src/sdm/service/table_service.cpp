        ...
        if (existing.has_value()) {
            if (existing->spec.operation_id != param.operation_id) {
                return Status::ALREADY_EXIST(fmt::format(
                    "table_id {} already exists with operation_id {}",
...
Status TableService::drop_table(const DropTableParam& param) {
    ...
    Status status = store_->write_with([&](SdmStoreTxn& txn) -> Status {
        ...
        if (existing->state.phase == TablePhase::DELETED &&
            ...
            return Status::OK(fmt::format(
                "the table has been droped, table_id:{}", existing->table_id));
        }

        if (existing->state.desired == TableDesired::ABSENT) {
            return Status::OK(fmt::format(
                "the table's desired is absent, so it is dropping. table_id:{}",
                existing->table_id));
        }

        if (existing->state.phase != TablePhase::READY) {
            return Status::ALREADY_EXIST(fmt::format(
                "table_id {} is not READY for drop", param.table_id));
        }
...
Status TableService::get_table_status(const GetTableStatusParam& param,
    ...
    TableOr existing;
    RETURN_IF_INVALID_STATUS(store_->read_with([&](const SdmStoreTxn& txn) {
        return txn.get_table(param.table_id, existing);
    }))
    if (existing.is_empty()) {
        return Status::TABLE_NOT_FOUND(
            fmt::format("table_id {} not found", param.table_id));
...
Status TableService::reconcile_all() {
    ...
    for (Table& table : tables) {
        ...
        if (status.fail()) {
            LOG_WARN(
                "[TableService] reconcile table failed, table_id={}, msg={}",
...
Status TableService::reconcile_table(Table& table) {
    ...
    switch (table.state.desired) {
        case TableDesired::PRESENT: {
            Status status = finalize_present_table(table);
    ...
    LOG_WARN("[TableService] unknown table desired state");
    return mark_table_error(table,
                            Status::ERROR("unknown table desired state"));
}

Status TableService::finalize_present_table(Table& table) {
    return store_->write_with([&](SdmStoreTxn& txn) -> Status {
        TableOr current;
        ...
        if (current->state.desired != TableDesired::PRESENT ||
        ...
        RETURN_IF_INVALID_STATUS(
            ensure_all_shards_ok(txn, current.value(), all_shards_ready_flag))
        ...
        current->state.phase =
            all_shards_ready_flag ? TablePhase::READY : TablePhase::CREATING;
...
Status TableService::ensure_all_shards_ok(SdmStoreTxn& txn, Table& table,
                                          bool& all_shards_ok) {
    all_shards_ok = true;
    for (ShardIndex shard_index = 0; shard_index < table.spec.shard_count;
        ...
        // 这里要去确保 replica_group 也创建完成了
        {
            ...
            bool group_phase_ok =
                !group.is_empty() &&
                (group->phase == ReplicaGroupPhase::TABLE_RECONCILE ||
                 group->phase ==
                     ReplicaGroupPhase::REPLICA_GROUP_RECONCILE);  // TODO
            if (!group_phase_ok ||
...
Status TableService::finalize_absent_table(Table& table) {
    return store_->write_with([&](SdmStoreTxn& txn) -> Status {
        ...
        if (current->state.desired != TableDesired::ABSENT ||
        ...
        RETURN_IF_INVALID_STATUS(
            ensure_all_shards_deleted(txn, current.value(), all_shards_deleted))
...
Status TableService::ensure_all_shards_deleted(SdmStoreTxn& txn, Table& table,
    ...
    for (ShardIndex shard_index = 0; shard_index < table.spec.shard_count;
        ...
        for (const Replica& replica : replicas) {
            if (replica.state.phase == ReplicaPhase::ERROR) {
                table.state.phase = TablePhase::FAILED;
                table.state.last_error_msg = fmt::format(
                    "replica delete failed, replica.state.last_error_msg:{}",
                    replica.state.last_error_msg);
...
Status TableService::mark_table_error(Table& table, const Status& status) {
    return store_->write_with([&](SdmStoreTxn& txn) -> Status {
        TableOr current;
        RETURN_IF_INVALID_STATUS(txn.get_table(table.table_id, current))
        if (current.is_empty()) {
        ...
        current->state.phase = TablePhase::FAILED;
        current->state.last_error_msg = status.to_string();
        current->state.update_ts = func::get_current_ts_ms();
        table = *current;
        return txn.put_table(*current);
    });
