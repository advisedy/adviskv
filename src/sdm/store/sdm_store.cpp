...
SdmStore::SdmStore(const MetaStoreParam& param)
        : SdmStore(param, std::make_unique<SdmRuntimeIndex>(), std::make_unique<SdmRuntimeStore>()) {}

SdmStore::SdmStore(const MetaStoreParam& param, std::unique_ptr<SdmRuntimeIndex> runtime_index,
                   std::unique_ptr<SdmRuntimeStore> runtime_store) {
    runtime_index_ = std::move(runtime_index);
    runtime_store_ = std::move(runtime_store);
    if (const auto* memory_param = std::get_if<MemoryMetaStoreParam>(&param)) {
        UNUSED(memory_param);
        meta_store_ = std::make_unique<MemoryMetaStore>();
    } else if (const auto* persist_param = std::get_if<PersistentMetaStoreParam>(&param)) {
        meta_store_ = std::make_unique<PersistentMetaStore>(std::filesystem::path(persist_param->data_dir));
    }
}

SdmStore::SdmStore(std::unique_ptr<ISdmMetaStore> meta_store, std::unique_ptr<SdmRuntimeStore> runtime_store,
...
Status SdmStore::init() {
    ...
    RETURN_IF_NULLPTR(runtime_store_, "sdm runtime store is nullptr")
    RETURN_IF_NULLPTR(runtime_index_, "sdm runtime index is nullptr")

    Status status = meta_store_->init();
    if (status.fail()) {
        LOG_ERROR("[SdmStore] metastore init failed: {}", status.to_string());
        return status;
    }
    status = runtime_store_->init();
    if (status.fail()) {
        LOG_ERROR("[SdmStore] runtime store init failed: {}", status.to_string());
        return status;
    }
    status = rebuild_runtime_index();
    if (status.fail()) {
        LOG_ERROR("[SdmStore] rebuild runtime index failed: {}", status.to_string());
        return status;
    }
    return Status::OK();
}

Status SdmStore::read_with(const std::function<Status(const SdmStoreTxn&)>& func) const {
    if (func == nullptr) {
...
Status SdmStore::write_with(const std::function<Status(SdmStoreTxn&)>& func) {
    ...
    std::unique_ptr<ISdmMetaStore> clone_meta = meta_store_->clone_memory_snapshot();
    if (clone_meta == nullptr) {
        return Status::ERROR("write_with clone meta failed");
    }
    std::unique_ptr<SdmRuntimeStore> clone_runtime_store = runtime_store_->clone();
    if (clone_runtime_store == nullptr) {
        return Status::ERROR("write_with clone runtime_store failed");
    }
    ...
    SdmStore clone_store(std::move(clone_meta), std::move(clone_runtime_store), std::move(clone_index));
    ...
    if (clone_store.meta_dirty_) {
        status = meta_store_->commit_memory_snapshot(std::move(clone_store.meta_store_));
        RETURN_IF_INVALID_STATUS(status)
    }
    if (clone_store.runtime_store_dirty_) {
        runtime_store_ = std::move(clone_store.runtime_store_);
    }
    if (clone_store.runtime_index_dirty_) {
        runtime_index_ = std::move(clone_store.runtime_index_);
    }
    return Status::OK();
}

void SdmStore::mark_meta_dirty() { meta_dirty_ = true; }

void SdmStore::mark_runtime_store_dirty() { runtime_store_dirty_ = true; }

void SdmStore::mark_runtime_index_dirty() { runtime_index_dirty_ = true; }
...
Status SdmStore::put_table(const Table& table) {
    ...
    RETURN_IF_INVALID_STATUS(maybe_repair_runtime_index(runtime_index_->on_table_upsert(old_ptr.get(), table)))
    mark_meta_dirty();
    mark_runtime_index_dirty();
    return Status::OK();
}
...
Status SdmStore::get_table_by_name(const std::string& db_name, const std::string& table_name, TableOr& out) const {
    TableID table_id;
    RETURN_IF_INVALID_STATUS(runtime_index_->find_table_by_name(db_name, table_name, table_id))
    TablePtr table;
    RETURN_IF_INVALID_STATUS(meta_store_->get_table(table_id, table))
    out = table ? TableOr{*table} : TableOr{};
    return Status::OK();
...
Status SdmStore::list_tables(std::vector<Table>& out) const {
    ...
    std::sort(out.begin(), out.end(), [](const Table& lhs, const Table& rhs) { return lhs.table_id < rhs.table_id; });
    return Status::OK();
}
...
Status SdmStore::delete_table(TableID table_id) {
    ...
    RETURN_IF_INVALID_STATUS(maybe_repair_runtime_index(runtime_index_->on_table_delete(*old_ptr)))
    mark_meta_dirty();
    mark_runtime_index_dirty();
    return Status::OK();
}
...
Status SdmStore::put_node(const Node& node) {
    NodePtr old_ptr;
    RETURN_IF_INVALID_STATUS(runtime_store_->get_node(node.id, old_ptr))
    RETURN_IF_INVALID_STATUS(runtime_store_->upsert_node(node))
    RETURN_IF_INVALID_STATUS(maybe_repair_runtime_index(runtime_index_->on_node_upsert(old_ptr.get(), node)))
    mark_runtime_store_dirty();
    mark_runtime_index_dirty();
    return Status::OK();
}
...
Status SdmStore::get_node(const NodeID& node_id, NodeOr& out) const {
    NodePtr node;
    RETURN_IF_INVALID_STATUS(runtime_store_->get_node(node_id, node))
...
Status SdmStore::list_nodes(std::vector<Node>& out) const {
    ...
    RETURN_IF_INVALID_STATUS(runtime_store_->list_nodes(nodes))
    out.clear();
    ...
    std::sort(out.begin(), out.end(), [](const Node& lhs, const Node& rhs) { return lhs.id < rhs.id; });
    return Status::OK();
}

Status SdmStore::list_nodes_by_resource_pool(const std::string& pool_name, std::vector<Node>& out) const {
    std::vector<NodeID> node_ids;
    RETURN_IF_INVALID_STATUS(runtime_index_->list_nodes_by_resource_pool(pool_name, node_ids))
...
Status SdmStore::get_resource_pool(const std::string& name, ResourcePoolOr& out) const {
...
Status SdmStore::list_resource_pools(std::vector<ResourcePool>& pools) const {
    ...
    std::sort(pools.begin(), pools.end(),
              [](const ResourcePool& lhs, const ResourcePool& rhs) { return lhs.name < rhs.name; });
    return Status::OK();
}
...
Status SdmStore::put_replica(const Replica& replica) {
    ReplicaPtr old_ptr;
    RETURN_IF_INVALID_STATUS(meta_store_->get_replica(replica.replica_id, old_ptr))
    RETURN_IF_INVALID_STATUS(meta_store_->upsert_replica(replica))
    RETURN_IF_INVALID_STATUS(maybe_repair_runtime_index(runtime_index_->on_replica_upsert(old_ptr.get(), replica)))
    mark_meta_dirty();
    mark_runtime_index_dirty();
    return Status::OK();
}
...
Status SdmStore::put_replicas(const std::vector<Replica>& replicas) {
    ...
    RETURN_IF_INVALID_CONDITION(old_ptrs.size() == replicas.size(), "old_ptr size should == replicas size")

    RETURN_IF_INVALID_STATUS(meta_store_->upsert_replicas(replicas))
    for (int i = 0, siz = replicas.size(); i < siz; i++) {
        Status status = runtime_index_->on_replica_upsert(old_ptrs[i].get(), replicas[i]);
        if (status.fail()) {
...
Status SdmStore::get_replica(const ReplicaID& replica_key, ReplicaOr& out) const {
    ReplicaPtr replica;
...
Status SdmStore::del_replica(const ReplicaID& replica_key) {
    ...
    RETURN_IF_INVALID_STATUS(maybe_repair_runtime_index(runtime_index_->on_replica_delete(*old_ptr)))
    mark_meta_dirty();
    mark_runtime_index_dirty();
    return Status::OK();
}
...
Status SdmStore::list_replicas_by_shard(const ShardID& shard_id, std::vector<Replica>& out) const {
    std::vector<ReplicaID> replica_ids;
    RETURN_IF_INVALID_STATUS(runtime_index_->list_replicas_by_shard(shard_id, replica_ids));
...
Status SdmStore::list_replicas_by_node(NodeID node_id, std::vector<Replica>& out) const {
    std::vector<ReplicaID> replica_ids;
    RETURN_IF_INVALID_STATUS(runtime_index_->list_replicas_by_node(node_id, replica_ids));
...
Status SdmStore::put_shard_route(const ShardRoute& route) {
    RETURN_IF_INVALID_STATUS(runtime_store_->put_shard_route(route))
    mark_runtime_store_dirty();
    return Status::OK();
}

Status SdmStore::get_shard_route(const ShardID& shard_id, ShardRouteOr& out) const {
    ShardRoutePtr route;
    RETURN_IF_INVALID_STATUS(runtime_store_->get_shard_route(shard_id, route))
...
Status SdmStore::delete_shard_route(const ShardID& shard_id) {
    ...
    RETURN_IF_INVALID_STATUS(runtime_store_->delete_shard_route(shard_id))
    mark_runtime_store_dirty();
    return Status::OK();
}

Status SdmStore::del_shard_route_entry(const ShardID& shard_id, const ReplicaID& replica_id) {
    ShardRoutePtr route;
    RETURN_IF_INVALID_STATUS(runtime_store_->get_shard_route(shard_id, route))
    if (route == nullptr) {
        return Status::OK();
    }
    RETURN_IF_INVALID_STATUS(runtime_store_->del_shard_route_entry(shard_id, replica_id))
    mark_runtime_store_dirty();
    return Status::OK();
}
...
Status SdmStore::get_replica_group(const ShardID& shard_id, ReplicaGroupOr& out) const {
...
Status SdmStore::list_replica_groups(std::vector<ReplicaGroup>& out) const {
    ...
    std::sort(out.begin(), out.end(), [&less_shard_id](const ReplicaGroup& lhs, const ReplicaGroup& rhs) {
...
Status SdmStore::maybe_repair_runtime_index(Status index_status) {
    ...
    LOG_ERROR("runtime index update failed, rebuild index, msg={}", index_status.msg());
...
Status SdmStore::rebuild_runtime_index() {
    ...
    std::vector<NodePtr> nodes;
    status = runtime_store_->list_nodes(nodes);
    RETURN_IF_INVALID_STATUS(status)
    for (const NodePtr& node : nodes) {
        if (!node) continue;
