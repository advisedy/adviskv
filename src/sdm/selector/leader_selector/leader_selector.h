...
public:
    ...
    virtual Status select_leader(const std::vector<NodePtr>& nodes, NodePtr& leader) const = 0;
};
...
//    Status select_leader(const std::vector<NodeStats>& nodes, NodeID& leader) const override;
      Status select_leader(const std::vector<NodePtr>& nodes, NodePtr& leader) const override;
