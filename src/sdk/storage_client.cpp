...
Status StorageClient::select_leader_replica(const RouteInfo& route, RouteReplica* selected) {
    ...
    RETURN_IF_INVALID_CONDITION(!route.replicas.empty(), "route replicas should not empty")

    for (const RouteReplica& replica : route.replicas) {
        if (replica.role != ReplicaRole::LEADER) {
            continue;
        }
        RETURN_IF_INVALID_CONDITION(!replica.endpoint.ip.empty(), "leader replica ip invalid")
        RETURN_IF_INVALID_CONDITION(replica.endpoint.port > 0, "leader replica port invalid")
        *selected = replica;
        return Status::OK();
    }
...
rpc::StorageService::Stub* StorageClient::make_stub(const Endpoint& endpoint) const {
...
Status StorageClient::put(const RouteInfo& route, const Key& key, const Value& value) const {
    ...
    if (status.fail()) {
        ...
        ADVISKV_SDK_LOG(LogLevel::WARN,
                        ...
                        "shard_id={}, key={}, status={}",
                        route.table_id, route.shard_id, key, status.to_string());
        return status;
    }
    ...
    context.set_deadline(std::chrono::system_clock::now() + Milliseconds(conf_.storage_timeout_ms));
    ...
    if (!grpc_status.ok()) {
        ...
        ADVISKV_SDK_LOG(LogLevel::ERROR,
                        ...
                        "msg={}",
                        leader.endpoint.ip, leader.endpoint.port, route.table_id, route.shard_id,
                        leader.replica_id.to_string(), key, static_cast<int>(grpc_status.error_code()),
                        grpc_status.error_message());
        return Status::ERROR(fmt::format("Storage Put RPC failed, grpc code = {}, msg = {}",
    ...
    if (Status status = decode_base_rsp_status(response.base_rsp()); status.fail()) {
        ADVISKV_SDK_LOG(LogLevel::WARN,
                        ...
                        "code={}, msg={}",
                        leader.endpoint.ip, leader.endpoint.port, route.table_id, route.shard_id,
                        leader.replica_id.to_string(), key, response.base_rsp().code(), response.base_rsp().msg());
        return status;
    }
...
Status StorageClient::del(const RouteInfo& route, const Key& key) const {
    ...
    if (!grpc_status.ok()) {
        ...
        ADVISKV_SDK_LOG(LogLevel::ERROR,
                        ...
                        leader.replica_id.to_string(), key, static_cast<int>(grpc_status.error_code()),
                        grpc_status.error_message());
        return Status::ERROR(fmt::format("Storage Delete RPC failed, grpc code = {}, msg = {}",
                                         static_cast<int>(grpc_status.error_code()), grpc_status.error_message()));
    }
    if (Status status = decode_base_rsp_status(response.base_rsp()); status.fail()) {
        ADVISKV_SDK_LOG(LogLevel::WARN,
...
Status StorageClient::get(const RouteInfo& route, const Key& key, Value* value) const {
    ...
    if (!grpc_status.ok()) {
        ADVISKV_SDK_LOG(LogLevel::ERROR,
                        ...
                        leader.replica_id.to_string(), key, static_cast<int>(grpc_status.error_code()),
                        grpc_status.error_message());
        return Status::ERROR(fmt::format("Storage Get RPC failed, grpc code = {}, msg = {}",
                                         static_cast<int>(grpc_status.error_code()), grpc_status.error_message()));
    }
