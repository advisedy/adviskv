...
当前 V1 已经跑通一条完整主链路：建库建表、表分片、多副本、调整副本数、异常副本替换、route 生成、SDK 读写、Raft 复制、WAL、snapshot、crash recovery、gtest、E2E、benchmark 和 metrics 文本报告。它更适合作为分布式 KV 架构和本地实验工具链的可运行原型，而不是可直接生产部署的数据库。
...
这里记录一组本地端到端 benchmark，链路覆盖 `SDK -> SDM route -> Storage leader -> Raft/WAL/KV`。结果来自下面这台机器和本地多进程集群，只代表当前 V1 的一次测试结果。
...
报告会列出本次 benchmark 期间有增量的 latency histogram 和 counter delta，包括 benchmark 客户端里的 `sdk_*` 行，适合本地调优和回归比较。
...
- Storage 和 SDK benchmark metrics 已可采样，Meta 还没有同等级别的 metrics/health 暴露。
- Benchmark 结果来自本地 V1 测试，适合做调优和回归对比，不代表生产环境性能。
