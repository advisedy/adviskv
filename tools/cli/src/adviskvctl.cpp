...
class AdvisKvCtl {
   public:
    explicit AdvisKvCtl(AdvisKvCtlConfig config)
        : config_(std::move(config)), meta_client_(config_.meta) {}
    ...
    bool execute(const std::vector<std::string>& tokens) {
        ...
        if (command == "create_db" || command == "create-db") {
            ...
            create_table(tokens);
        } else if (command == "get_table" || command == "get-table" ||
                   command == "status") {
            get_table(tokens);
   ...
   private:
    adviskv::sdk::KVClient make_kv_client(const std::string& db,
    ...
    adviskv::sdk::SdmRouteClient make_route_client(
    ...
    void create_table(const std::vector<std::string>& tokens) const {
        ...
        int32_t shards = 0;
        int32_t replicas = 0;
        if (!parse_i32(tokens[3], &shards) ||
            !parse_i32(tokens[4], &replicas)) {
            fmt::print("shards and replicas should be integers\n");
            return;
        }
        TableID table_id = -1;
        Status status = meta_client_.create_table(
            tokens[1], tokens[2], shards, replicas, &table_id, tokens[5]);
        if (status.ok()) {
    ...
    bool fetch_table(const std::string& db, const std::string& table,
    ...
    void print_table(const TableInfo& table_info) const {
        fmt::print(
            ...
            table_info.db_id, table_info.table_id, table_info.shard_count,
            table_info.replica_count,
            table_state_to_string(table_info.table_state),
    ...
    void wait_table(const std::vector<std::string>& tokens) const {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            if (!fetch_table(tokens[1], tokens[2], &table_info)) return;
    ...
    void route(const std::vector<std::string>& tokens) const {
        ...
        fmt::print("  table: {}.{}\n", tokens[1], tokens[2]);
        fmt::print("  key:   {}\n", tokens[3]);
        fmt::print("  id:    table_id={} shard_id={}\n", route_info.table_id,
                   route_info.shard_id);
        fmt::print("  replicas:\n");
        for (const auto& replica : route_info.replicas) {
            fmt::print("    - {}:{}  {}\n", replica.endpoint.ip,
                       replica.endpoint.port,
                       replica_role_to_string(replica.role));
        }
    }
    ...
    void demo(const std::vector<std::string>& tokens) const {
        if (tokens.size() < 2 || tokens.size() > 3) {
            fmt::print(
                "usage: demo <status|start|stop|kill|restart> [service]\n");
            return;
        }
        ...
        std::string command = "cd " + adviskv::project_root_dir().string() +
                              " && python3 scripts/local_cluster.py " + action;
        if (tokens.size() == 3) {
...
bool parse_global_args(int argc, char* argv[], std::string* conf_file,
                       std::vector<std::string>* command_tokens) {
    if (conf_file == nullptr || command_tokens == nullptr) return false;
    *conf_file = "conf/client.yaml";
    ...
    if (!command_tokens->empty()) {
        fmt::print(stderr,
                   "adviskvctl only supports interactive mode; run without "
                   "command args and type commands in the shell\n");
        return false;
    }
    return true;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!load_config(conf_file, &config)) return 1;

    fmt::print("connected meta {}:{}, sdm {}:{}\n", config.meta.endpoint.ip,
               config.meta.endpoint.port, config.sdk.sdm_host,
               config.sdk.sdm_port);
