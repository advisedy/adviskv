这个分支是专门给sdsdk留的，因为storage这边采用的raft，所以发现这个sdsdk已经没有存在的必要了。属于是过度设计了，按照现在的思路，storage这边只需要搞一个backgroundtask其实就可以了。但是直接删掉sdsdk又感觉太可惜了，于是专门安排一个分支给他留作纪念，万一以后的架构里面又需要它了呢？
致辞: 2026.5.3
